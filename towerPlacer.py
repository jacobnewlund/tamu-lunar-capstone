...
def segmentPlacer(lats, longs, dists, dirs, segType): # generates tower placements per segment spaced evenly
    ...
    if segType == "end":
        towerLocs[-1] = [towerLocs[-1][0], "curve"] # retroactively change the last transition to a curve since end segment is technically a curve. don't like it? Me neither.
        towerLocs.append([[currentLat, currentLong], "curve"]) 
...
# let's get some tower stats before packaging it up into a .csv
print("Tower Location Statistics:")
print("{} towers placed...".format(len(towerLocs)))
print("versus {:.0f} towers expected.".format(estTowers))
if estTowers > len(towerLocs): # if the estimated tower count is greater than the actual...
    print("This horrible way of doing it saved us {:.0f} tower(s)! That's savings!!!".format(estTowers - len(towerLocs))) # yippee!!!!!!!!!
curveTowers = 1 # 1 since start counts as a curve tower and we're ignoring it otherwise. yes, this is hardcoding, and will probably bite us later
straightTowers = 0
transitionTowers = 0
curveDist = 0
straightDist = 0
lastTower = towerLocs[0]
for tower in towerLocs[1:]: # I did this really sloppily since I'm tired and want to be free from this thing
    lastTowerType = lastTower[1]
    towerType = tower[1]
    lastLat = lastTower[0][0]
    lastLong = lastTower[0][1]
    lat = tower[0][0]
    long = tower[0][1]
    # first, handle tower numbers
    if towerType == "curve":
        curveTowers += 1
    elif towerType == "straight":
        straightTowers += 1
    elif towerType == "transition":
        transitionTowers += 1
    else: #... then it's an end tower, which we consider a curve
        curveTowers += 1

    # then, handle span distances. there's a bunch of cases so I'm just handling each one individually since dumb-coding is fast
    if lastTowerType == towerType == "curve": # curve-curve
        curveDist += havDist(np.deg2rad(lastLat), np.deg2rad(lat), np.deg2rad(lastLong), np.deg2rad(long), radius)
    elif lastTowerType == towerType == "straight": # straight-straight
        straightDist += havDist(np.deg2rad(lastLat), np.deg2rad(lat), np.deg2rad(lastLong), np.deg2rad(long), radius)
    elif lastTowerType == "curve" and towerType == "transition": # curve-transition
        curveDist += havDist(np.deg2rad(lastLat), np.deg2rad(lat), np.deg2rad(lastLong), np.deg2rad(long), radius)
    elif lastTowerType == "transition" and towerType == "curve": # transition-curve
        curveDist += havDist(np.deg2rad(lastLat), np.deg2rad(lat), np.deg2rad(lastLong), np.deg2rad(long), radius)
    elif lastTowerType == "straight" and towerType == "transition": # straight-transition
        straightDist += havDist(np.deg2rad(lastLat), np.deg2rad(lat), np.deg2rad(lastLong), np.deg2rad(long), radius)
    elif lastTowerType == "transition" and towerType == "straight": # transition-straight
        straightDist += havDist(np.deg2rad(lastLat), np.deg2rad(lat), np.deg2rad(lastLong), np.deg2rad(long), radius)
    elif lastTowerType == "start" and towerType == "curve": # start-curve
        curveDist += havDist(np.deg2rad(lastLat), np.deg2rad(lat), np.deg2rad(lastLong), np.deg2rad(long), radius)
    elif lastTowerType == "curve" and towerType == "end": # curve-end
        curveDist += havDist(np.deg2rad(lastLat), np.deg2rad(lat), np.deg2rad(lastLong), np.deg2rad(long), radius)
    else: # uhh... uncaught case???
        print("hey you forgot a case", lastTowerType, towerType)
        raise Exception("you failed") # just cause an error i guess
    lastTower = tower

print("There are {} monorail towers, {} tramway towers, and {} 'transition towers'.".format(curveTowers, straightTowers, transitionTowers))
print("The path uses approx. {:.2f} meters of monorail span...".format(curveDist))
print("and approx. {:.2f} meters of tramway cable.".format(straightDist))
print("This makes a total distance of approximately {:.2f} meters!".format(straightDist+curveDist))

# price tag part comes... later.
...
df.to_csv('tower_locations.csv',index=False)